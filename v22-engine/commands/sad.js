...
module.exports = {
  name: 'sad',
  description: 'Send a sad reaction gif (nekos.best).',
  category: 'REACT',
  async execute(sock, msg, args) {
    ...
    try {
      ...
      await sock.sendMessage(jid, { video: { url: gif }, gifPlayback: true, caption: mention ? `🪅 *𝗠𝗘𝗦𝗛-𝗧𝗘𝗖𝗛*  ⟿  *🪅 SAD* @${mention.split('@')[0]}` : `🪅 *𝗠𝗘𝗦𝗛-𝗧𝗘𝗖𝗛*  ⟿  *🪅 SAD*`, mentions: mention ? [mention] : undefined }, { quoted: msg });
    } catch (err) {
      await sock.sendMessage(jid, { text: `❌ Failed to fetch sad gif: ${err.message}` }, { quoted: msg });
    }
  },
};
