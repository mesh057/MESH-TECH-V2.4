...
module.exports = {
  name: 'heart',
  description: 'Send a heart reaction gif to a mentioned member.',
  category: 'REACT',
  async execute(sock, msg) {
    ...
    try {
      ...
      await sock.sendMessage(jid, {
        video: { url: gif },
        gifPlayback: true,
        caption: mention ? `💗 *𝗠𝗘𝗦𝗛-𝗧𝗘𝗖𝗛*  ⟿  *HEART* @${mention.split('@')[0]}` : '💗 *𝗠𝗘𝗦𝗛-𝗧𝗘𝗖𝗛*  ⟿  *HEART*',
        mentions: mention ? [mention] : undefined,
      }, { quoted: msg });
    } catch (err) {
      await sock.sendMessage(jid, { text: `❌ Failed to fetch heart gif: ${err.message}` }, { quoted: msg });
    }
  },
};
