...
module.exports = {
  ...
  async execute(sock, msg) {
    ...
    const loading = await sock.sendMessage(jid, { text: "🏆 Fetching Ligue 1 top scorers..." }, { quoted: msg });

    try {
      const { data } = await axios.get(`${KEITH_BASE}/ligue1/scorers`);
      if (!data.status || !data.result?.scorers) throw new Error("No scorers available.");

      const scorers = data.result.scorers;
      let text = `⚽ *${data.result.competition || 'Ligue 1'} Top Scorers*\n\n`;
      scorers.forEach((s, i) => {
