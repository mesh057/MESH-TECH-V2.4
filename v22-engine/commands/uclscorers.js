...
module.exports = {
  ...
  async execute(sock, msg) {
    ...
    const loading = await sock.sendMessage(jid, { text: "🏆 Fetching Champions League top scorers..." }, { quoted: msg });

    try {
      const { data } = await axios.get(`${KEITH_BASE}/ucl/scorers`);
      if (!data.status || !data.result?.scorers) throw new Error("No scorers available.");

      const scorers = data.result.scorers;
      let text = `⚽ *${data.result.competition || 'Champions League'} Top Scorers*\n\n`;
      scorers.forEach((s, i) => {
